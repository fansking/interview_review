...
1. select
   ...
   将文件描述符fd注册在一个数组中，每当有事件发生时，遍历整个数组找到这个fd,将fd中数据复制到程序内存中。缺点是效率随数组长度而降低，且最大长度为1024

2. poll

   与select类似，但使用链表存储fd，突破1024限制

3. epoll

   epoll_ctl() 用于向内核注册新的描述符或者是改变某个文件描述符的状态。已注册的描述符在内核中会被维护在一棵红黑树上，通过回调函数内核会将 I/O 准备好的描述符加入到一个链表中管理，进程调用 epoll_wait() 便可以得到事件完成的描述符。支持水平触发(没处理就一致触发)和边缘触发(只在发生时触发一次)
